...
export default function ComplaintDetailsModal({ complaintId, onClose }) {
    ...
    const statusConfig = {
        ...
        assigned: { bg: 'bg-blue-50', text: 'text-blue-700', label: 'Assigned', dot: 'bg-blue-500' },
        in_progress: { bg: 'bg-purple-50', text: 'text-purple-700', label: 'In progress', dot: 'bg-purple-500' },
        pending_verification: { bg: 'bg-amber-50', text: 'text-amber-700', label: 'Pending verification', dot: 'bg-amber-500' },
        resolved: { bg: 'bg-emerald-50', text: 'text-emerald-700', label: 'Resolved', dot: 'bg-emerald-500' },
        reopened: { bg: 'bg-orange-50', text: 'text-orange-700', label: 'Reopened', dot: 'bg-orange-500' }
    };
    ...
    const {
        publicCode, title, description, status, category, department,
            ...
            >
                ...
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
                    <div className="flex items-center gap-3">
                        <span className="font-mono text-sm font-bold text-gray-500 tracking-wide">{publicCode}</span>
                        <span className={clsx("px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5", displayBadge.bg, displayBadge.text)}>
                            <span className={clsx("w-1.5 h-1.5 rounded-full", displayBadge.dot)}></span>
                            {displayBadge.label}
                        </span>
                    </div>
                    <button 
                                ...
                                {history && history.length > 0 ? history.map((item, index) => {
                                    ...
                                    return (
                                        <div key={item.id} className="relative flex items-start gap-4">
                                            <div className={clsx("w-6 h-6 rounded-full shrink-0 flex items-center justify-center z-10 ring-4 ring-white mt-1", hConf.bg, hConf.text)}>
                                                {item.toStatus === 'resolved' || item.toStatus === 'closed' ? (
                                                    <CheckCircle2 className="w-3.5 h-3.5" />
                                                ) : (
                                                    <div className={clsx("w-2 h-2 rounded-full", hConf.dot)}></div>
                                                )}
                                            </div>
                                            <div className="flex-1 pb-2">
                                                <div className="flex items-center justify-between mb-0.5">
                                                    <h4 className="font-bold text-gray-900 text-sm">{hConf.label}</h4>
                                                </div>
