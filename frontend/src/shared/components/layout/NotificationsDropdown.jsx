...
export default function NotificationsDropdown({ onClose, onUnreadCountChange, unreadCount }) {
    ...
    const loadNotifications = async (pageNumber) => {
        try {
            ...
            const res = await notificationsService.listNotifications({ page: pageNumber, limit: 10 });
    ...
    const handleNotificationClick = async (notif) => {
        ...
        if (!notif.isRead) {
            try {
                ...
                // We could re-fetch unread count here, or just decrement local state
                const res = await notificationsService.getUnreadCount();
                if (onUnreadCountChange) onUnreadCountChange(Number(res.data.data.count) || 0);
            } catch (error) {
