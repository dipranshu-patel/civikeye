...
function validateCreateComplaint(body, files) {
    ...
    if (!body.categoryId || typeof body.categoryId !== "string") {
        errors.push({ field: "categoryId", message: "A valid category (SLA category ID) is required." });
    }
    ...
    if (body.description && body.description.length > 600) {
        errors.push({ field: "description", message: "Description must not exceed 600 characters." });
    }
...
function validateExploreQuery(query) {
    ...
    if (query.sort && !VALID_SORTS_EXP.includes(query.sort)) {
        errors.push({ field: "sort", message: `Sort must be one of: ${VALID_SORTS_EXP.join(", ")}.` });
    }
...
function validateNearbyQuery(query) {
    ...
    if (isNaN(lng) || lng < -180 || lng > 180) {
        errors.push({ field: "lng", message: "A valid longitude is required." });
    }
    if (query.radius && (isNaN(radius) || radius < 100 || radius > 10_000)) {
        errors.push({ field: "radius", message: "Radius must be between 100 and 10000 metres." });
    }
    return errors;
}
...
function validateMyComplaintsQuery(query) {
    ...
    if (query.tab && !VALID_TABS.includes(query.tab)) {
        errors.push({ field: "tab", message: `Tab must be one of: ${VALID_TABS.join(", ")}.` });
    }
    if (query.sort && !VALID_SORTS_MY.includes(query.sort)) {
        errors.push({ field: "sort", message: `Sort must be one of: ${VALID_SORTS_MY.join(", ")}.` });
    }
    return errors;
}
