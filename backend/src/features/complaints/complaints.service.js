...
function isOverdue(complaint) {
    ...
    if (["resolved", "pending_verification"].includes(complaint.status))
        return false;
...
async function createComplaint({
    ...
    const slaDeadline = slaDays
        ? new Date(Date.now() + slaDays * 86_400_000)
        : null;

    const duplicates = await repo.findDuplicateCandidates({
        lat: latitude,
        lng: longitude,
        categoryId,
    });
    ...
    if (issueType === "authority_required") {
        const { rows: deptRows } = await query(
            "SELECT user_id FROM departments WHERE id = $1",
            [departmentId],
        );
...
async function exploreComplaints({
    search,
    status,
    issueType,
    categoryId,
...
async function getNearbyComplaints({ lat, lng, radius }) {
    const rows = await repo.findNearbyComplaints({
        lat,
        lng,
        radiusMetres: radius,
    });
...
async function getSimilarComplaints({ lat, lng, categoryId }) {
    const rows = await repo.findSimilarComplaints({
        lat,
        lng,
        categoryId,
        excludeId: null,
...
async function getMyComplaints({ reporterId, tab, search, sort, page, limit }) {
    ...
    return {
        summary: {
            ...
            avgOverdueHours: summary.avg_overdue_hours
                ? parseFloat(summary.avg_overdue_hours)
                : null,
        },
...
async function addUpvote(complaintId, userId) {
    ...
    const newCount = await withTransaction((client) =>
        repo.insertUpvote(client, complaintId, userId),
    );
    ...
    if (UPVOTE_REPORTER_MILESTONES.includes(newCount)) {
        notify(null, {
            ...
            data: {
                ...
                publicCode: complaint.public_code,
                complaintId,
            },
            entityType: "complaint",
    ...
    if (
        UPVOTE_DEPT_MILESTONES.includes(newCount) &&
        complaint.issue_type === "authority_required"
    ) {
        const { rows: deptRows } = await query(
            "SELECT user_id FROM departments WHERE id = $1",
...
async function removeUpvote(complaintId, userId) {
    ...
    const newCount = await withTransaction((client) =>
        repo.deleteUpvote(client, complaintId, userId),
    );
    if (newCount === null)
        throw new AppError(
            "NOT_UPVOTED",
            "You have not upvoted this complaint.",
            404,
        );
    return { upvoteCount: newCount };
}
...
async function getDashboard(userId, { lat, lng }) {
    ...
    return {
        ...
        verificationRequests:
            data.nearbyPendingVerifications.map(formatComplaint),
