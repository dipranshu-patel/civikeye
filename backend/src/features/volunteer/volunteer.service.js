    ...
    if (result.error) {
        const MAP = {
            TASK_NOT_FOUND: [
                404,
                "TASK_NOT_FOUND",
                "Volunteer task not found.",
            ],
            TASK_NOT_OPEN: [
                409,
                "TASK_NOT_OPEN",
                "This task is no longer open for claiming.",
            ],
            ALREADY_CLAIMED: [
                409,
                "ALREADY_CLAIMED",
            ...
            NOT_COMMUNITY_FIXABLE: [
                422,
                "NOT_COMMUNITY_FIXABLE",
                "Only community-fixable complaints can be claimed.",
            ],
            REPORTER_CANNOT_CLAIM: [
                403,
                "REPORTER_CANNOT_CLAIM",
                "You cannot volunteer for a complaint you reported.",
            ],
        };
        const [status, code, msg] = MAP[result.error] ?? [
            500,
            "UNKNOWN_ERROR",
            "Unexpected error.",
        ];
...
async function submitTaskCompletion({ taskId, volunteerId, note, proofFile }) {
    const result = await repo.submitTaskCompletion({
        taskId,
        volunteerId,
        note,
        proofFile,
    ...
    if (result.error) {
        const MAP = {
            ASSIGNMENT_NOT_FOUND: [
                404,
                "ASSIGNMENT_NOT_FOUND",
                "You have not claimed this task.",
            ],
            ASSIGNMENT_NOT_ACTIVE: [
                409,
                "ASSIGNMENT_NOT_ACTIVE",
                "This assignment is not in active status.",
            ],
        };
        const [status, code, msg] = MAP[result.error] ?? [
            500,
...
function formatLeaderboardEntry(row) {
    ...
    return {
        ...
        verifyRatePct: row.verify_rate_pct
            ? parseFloat(row.verify_rate_pct)
            : null,
    };
}
