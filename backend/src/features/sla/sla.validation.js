...
function validateCreateSlaCategory(body) {
    ...
    if (!body.name || typeof body.name !== "string" || body.name.trim().length < 2) {
        errors.push({ field: "name", message: "SLA category name must be at least 2 characters." });
    }
    if (body.name && body.name.trim().length > 150) {
        errors.push({ field: "name", message: "SLA category name must not exceed 150 characters." });
    }

    if (!body.departmentId || typeof body.departmentId !== "string") {
        errors.push({ field: "departmentId", message: "A valid department ID is required." });
    }

    const days = Number(body.slaDurationDays);
    if (!body.slaDurationDays || isNaN(days) || !Number.isInteger(days) || days < 1) {
        errors.push({ field: "slaDurationDays", message: "SLA duration must be a positive integer (days)." });
    }
    if (days > 365) {
        errors.push({ field: "slaDurationDays", message: "SLA duration cannot exceed 365 days." });
    }
...
function validateUpdateSlaCategory(body) {
    ...
    if (body.departmentId !== undefined && typeof body.departmentId !== "string") {
        errors.push({ field: "departmentId", message: "A valid department ID is required." });
    }
