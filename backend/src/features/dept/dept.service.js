...
function formatComplaint(row) {
    ...
    const isOverdue =
        deadline && !["resolved", "pending_verification"].includes(row.status)
            ? deadline < now
            : false;
...
function formatPerformance(row) {
    if (!row) return null;
    return {
        resolutionRate: row.resolution_rate_pct
            ? parseFloat(row.resolution_rate_pct)
            : null,
        avgResolutionDays: row.avg_resolution_days
            ? parseFloat(row.avg_resolution_days)
            : null,
        slaTargetDays: row.sla_target_days ?? null,
        overdueRate: row.overdue_rate_pct
            ? parseFloat(row.overdue_rate_pct)
            : null,
...
async function getDeptComplaints({
    departmentId,
    tab = "assigned",
    search,
    page = 1,
...
async function updateComplaintStatus({
    ...
    const complaint = await repo.findDeptComplaintById(
        complaintId,
        departmentId,
    );
    if (!complaint) {
    ...
    if (toStatus === "pending_verification") {
        if (!note?.trim()) {
            throw new AppError(
                "NOTE_REQUIRED",
                "A resolution note is required when submitting proof.",
                422,
            );
        }
        if (!workPhotos?.length) {
            throw new AppError(
                "PHOTO_REQUIRED",
                "At least one work photo is required as proof of resolution.",
                422,
            );
        }
    }
